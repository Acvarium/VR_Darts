...
var last_left_position = Vector3()
var current_left_velocity = Vector3()

var last_right_position = Vector3()
var current_right_velocity = Vector3()
...
func pick_up_dart(is_right = true):
	if (is_right and right_obj != null):
		return
	if (!is_right and left_obj != null):
		return
	
	var new_dart = main_node.select_dart()
	if !new_dart:
		return
	new_dart.get_parent().remove_child(new_dart)
	if is_right:
		$RightTouchController/pos.add_child(new_dart)
		right_obj = new_dart
	else:
		$LeftTouchController.add_child(new_dart)
		left_obj = new_dart
	new_dart.activate(Vector3(), false)
	new_dart.transform = Transform()
	new_dart.visible = true

func throw_dart(is_right = true):
	var dart_to_throw = right_obj
	if !is_right:
		dart_to_throw = left_obj
		left_obj = null
	else:
		right_obj = null
	if dart_to_throw:
		var dart_transform = $RightTouchController.global_transform
		var vel = get_smooth_vel(is_right)#current_right_velocity
		if !is_right:
#			vel = current_left_velocity
			dart_transform = $LeftTouchController.global_transform
		main_node.throw_dart(dart_to_throw, vel, dart_transform)
		print(vel)
func _ready():
	_initialize_ovr_mobile_arvr_interface();

func log_velocity(current_l, current_r):
	if r_vel_log.size() <= vel_log_index:
		r_vel_log.append(Vector3())
	if l_vel_log.size() <= vel_log_index:
		l_vel_log.append(Vector3())
	
	r_vel_log[vel_log_index] = current_r
	l_vel_log[vel_log_index] = current_l
	
	vel_log_index += 1
	if vel_log_index >= vel_log_num:
		vel_log_index = 0
		
func get_smooth_vel(is_right = true):
	var value_list = []
	var list_len = r_vel_log.size()
	for i in range(list_len):
		var n = vel_log_index + i
		if n >= list_len:
			n = i - (vel_log_index + 1)
		if (is_right):
			value_list.append(r_vel_log[n])
		else:
			value_list.append(l_vel_log[n])
	var _divisor  = 0
	var _sum = Vector3()
	for i in range(value_list.size()):
		var mult = value_list.size() - i
		_divisor += mult
		_sum += value_list[i] * float(mult)
	return _sum / float(_divisor)

func _physics_process(delta):
	var current_left_pos = $LeftTouchController.global_transform.origin
	var current_right_pos = $RightTouchController.global_transform.origin

	current_left_velocity = (current_left_pos - last_left_position) / delta
	current_right_velocity = (current_right_pos - last_right_position) / delta
	
	log_velocity(current_left_velocity, current_right_velocity)
	
	last_left_position = current_left_pos
	last_right_position = current_right_pos
...
func _on_LeftTouchController_button_pressed(button):
	main_node.button_pressed_released(button, 'l')
...
func _on_RightTouchController_button_pressed(button):
	if (button == 15):
		pick_up_dart()
	main_node.button_pressed_released(button, 'r')
	if (button != CONTROLLER_BUTTON.YB): return;
...
func _on_RightTouchController_button_release(button):
	if (button == 15):
		throw_dart()
	main_node.button_pressed_released(button, 'r', false)
	if (button != CONTROLLER_BUTTON.YB): return;
